...
class LoyaltiesTest extends TestCase {
    ...
    public function testUpdateLoyaltyMemberCardBalanceBy1000Points() {
        ...
        $this->assertEquals($filteredSnapshot, $filteredResponse, 'Error during test with updating loyalty member card balance');
    }
    ...
    public function testListLoyaltyMemberCardTransactions() {
        ...
        [$filteredSnapshot, $filteredResponse] = validatePayloads($snapshot, $transactions, $keysToRemove);

        $this->assertEquals($filteredSnapshot, $filteredResponse, 'Error during test with listing loyalty member card transactions');
    }
}
?>